            ...
            get
            {
                ...
                if (expiration.CompareTo(DateTime.Now) < 0) // expired!
                {
                    if (medicalCourse.ExpirationDate.CompareTo(physicianVisit.ExpirationDate) < 0)
                        return new CurrencyStatusItem(Resources.Profile.BasicMedCurrencyTitle, expiration.ToShortDateString(), CurrencyState.NotCurrent, Resources.Profile.BasicMedErrNoMedicalCourseWithin2Years);
                    else
                        return new CurrencyStatusItem(Resources.Profile.BasicMedCurrencyTitle, expiration.ToShortDateString(), CurrencyState.NotCurrent, Resources.Profile.BasicMedErrNoPhysicalWithin4Years);
                }
                else
                    return new CurrencyStatusItem(Resources.Profile.BasicMedCurrencyTitle, expiration.ToShortDateString(), (expiration.CompareTo(DateTime.Now.AddCalendarMonths(1)) < 0) ? CurrencyState.GettingClose : CurrencyState.OK, string.Empty);
            }
        }
        #endregion
