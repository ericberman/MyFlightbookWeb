                ...
                <div style="font-size:smaller">
                    <asp:MultiView ID="mvEarned" runat="server" ActiveViewIndex='<%# ViewIndexForBadge((MyFlightbook.Achievements.Badge) Container.DataItem) %>'>
                        ...
                        <asp:View ID="vwAchievedWithFlight" runat="server">
                            <asp:HyperLink ID="lnkFlightEarned" Text='<%# Eval("EarnedDateString") %>' runat="server" NavigateUrl='<%# String.Format(System.Globalization.CultureInfo.InvariantCulture, "~/Member/FlightDetail.aspx/{0}", Eval("IDFlightEarned")) %>'></asp:HyperLink>
                        </asp:View>
                    </asp:MultiView>
                </div>
            </div>
