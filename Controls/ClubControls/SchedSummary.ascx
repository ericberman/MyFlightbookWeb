...
<asp:GridView ID="gvSchedSummary" CellPadding="4" ShowHeader="false" ShowFooter="false" runat="server" GridLines="None" AutoGenerateColumns="false">
    <Columns>
        <asp:TemplateField>
            <ItemTemplate>
                <div>
                    <asp:Label ID="lblDate" CssClass="boldface" runat="server" Text='<%# String.Format("{0:d}", Eval("LocalStart")) %>'></asp:Label>
                    <asp:Label ID="lblTime" runat="server" Text='<%# String.Format("{0:t} ({1})", Eval("LocalStart"), Eval("DurationDisplay")) %>'></asp:Label>
                </div>
                ...
                <asp:Label ID="lblName" CssClass="boldface" runat="server" Text='<%# String.IsNullOrEmpty(UserName) ? Eval("OwnerProfile.UserFullName") : "" %>'></asp:Label>
                <%# (String.IsNullOrEmpty(UserName) && String.IsNullOrEmpty(ResourceName)) ? " - " : "" %>
                <asp:Label ID="lblTail" CssClass="boldface" runat="server" Text='<%# String.IsNullOrEmpty(ResourceName) ? Eval("ResourceAircraft.DisplayTailnumber") : "" %>'></asp:Label>
                <%# (!String.IsNullOrEmpty(UserName) && !String.IsNullOrEmpty(ResourceName)) ? "" : " - " %>
