<%@ Control Language="C#" AutoEventWireup="true" Codebehind="mfbCurrency.ascx.cs" Inherits="Controls_mfbCurrency" %>
...
<asp:GridView ID="gvCurrency" CellPadding="2" CellSpacing="1" ShowHeader="false" runat="server" GridLines="None" AutoGenerateColumns="false" OnRowDataBound="gvCurrency_RowDataBound">
    <Columns>
        ...
        <asp:TemplateField ItemStyle-VerticalAlign="Top">
            <ItemTemplate>
                <div><asp:Label ID="lblStatus" runat="server" CssClass='<%# CSSForItem((MyFlightbook.Currency.CurrencyState) Eval("Status")) %>' Text='<%# Eval("Value") %>'></asp:Label></div>
