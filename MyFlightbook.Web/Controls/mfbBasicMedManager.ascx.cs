...
{
    public partial class mfbBasicMedManager : UserControl
    {
        ...
        protected void gvBasicMedEvents_RowCommand(object sender, CommandEventArgs e)
        {
            if (e != null && String.Compare(e.CommandName, "_Delete", StringComparison.OrdinalIgnoreCase) == 0)
            {
                ...
                BasicMedEvent bme = BasicMedEvent.EventsForUser(Page.User.Identity.Name).FirstOrDefault<BasicMedEvent>(bme2 => bme2.ID == id);
                if (bme != null)
                {
                    bme.Delete();
                    RefreshBasicMedEvents();
