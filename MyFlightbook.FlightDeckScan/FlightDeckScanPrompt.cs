...
- A screen has at most two elapsed-DURATION fields: one gate-to-gate (BLOCK or TRIP) and, separately, one airborne-only (FLIGHT). If you see only ONE duration field on the whole screen, it is BLOCK/TRIP - report it as blockRaw and leave flightTimeRaw null, even if it sits where a FLIGHT field would appear on a different airline's layout. Only report flightTimeRaw when a SECOND, separately-labeled duration field is also printed on screen.
